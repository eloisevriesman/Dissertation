# Script that fetches top 30 hot posts & all the comments from a list of subreddits and saves them in a new folder

import json
import os
import praw

def fetch_post_and_comments(subreddit_name, post_id, reddit, main_folder, i):
    """
    ...
    # Saves file inside a subreddit-specific folder, inside the main category folder
    subreddit_folder = os.path.join(main_folder, f'{subreddit_name}_data')
    os.makedirs(subreddit_folder, exist_ok=True)
    json_filename = os.path.join(subreddit_folder, f'{subreddit_name}_post_{i}_data.json')
...
def process_subreddit(subreddit_name, reddit, main_folder):
    """
    Processes the top 30 hot posts from a specified subreddit.
    ...
    for post in hot_posts:
        fetch_post_and_comments(subreddit_name, post.id, reddit, main_folder, i)
        i += 1
...
if __name__ == "__main__":
    config_file = 'subreddits/sports.txt'
    main(config_file)